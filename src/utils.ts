...
export function ensureArrays(p: PMProject) {
  p.modules = p.modules || [];
  for (const m of p.modules) m.tasks = m.tasks || [];
}

export function counts(p: PMProject) {
  let total = 0,
    todo = 0,
    done = 0,
...
/** Spójny, jednolinijkowy opis projektu do drzewa/tooltipów. */
export function formatProjectSummary(c: {
  modules: number;
  total: number;
  todo: number;
  done: number;
