...
export class WebViewBuilder {
  static build(config: WebViewConfig): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data:; style-src 'unsafe-inline'; script-src 'unsafe-inline';">
  <style>${BASE_STYLES}${config.styles || ''}</style>
</head>
<body>
  ${config.body}
  <script>const vscode = acquireVsCodeApi();${config.scripts || ''}</script>
</body>
</html>`;
  }
}
...
export const TaskPanelHTML = {
  body: `
    ...
    <div id="taskBox" class="hidden">
      ...
      <div style="opacity:.7;margin-top:6px;">
        ➡️ Prompty wpisuj teraz w terminalu <b>LLM Diff</b> (bez prefiksu <code>/</code>).
      </div>
    </div>`,
  ...
  scripts: `
    ...
    function taskAdd(){ 
      ...
      vscode.postMessage({type:'task.addFiles'}); 
    }
    ...
    function taskEnd(){ 
      vscode.postMessage({type:'task.end'}); 
    }
    ...
    window.addEventListener('message', e => {
      ...
      if(m.type==='updateTask'){
        ...
        if(m.task){
          ...
          document.getElementById('tname').textContent = m.task.name || '';
